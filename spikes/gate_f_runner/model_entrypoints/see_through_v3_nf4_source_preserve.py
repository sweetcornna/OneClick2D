"""Source-preserving entrypoint for the pinned See-through V3 NF4 spike."""
...
def _source_preserve_visible_pixels(source_directory: Path) -> None:
    ...
    for index, name in enumerate(PART_NAMES):
        ...
        transmittance *= 1.0 - alpha.astype(np.float32) / 255.0

    for index, name in enumerate(PART_NAMES):
        path = source_directory / f"{name}.png"
    ...
    union_alpha = np.rint((1.0 - transmittance) * 255.0).clip(0, 255).astype(np.uint8)
