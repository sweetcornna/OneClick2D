"""Fixed bounded device policy for the v4 NF4 Marigold entrypoint."""
...
POLICY_ID = "see-through.v4.nf4-marigold-bounded-offload.v1"
...
def _hook_execution_devices(module: object) -> list[str]:
    modules: list[object]
    ...
    if callable(inventory):
        ...
        modules = [module]
    devices = {
        _device_name(execution_device)
        for item in modules
        if (execution_device := getattr(getattr(item, "_hf_hook", None), "execution_device", None)) is not None
    }
    return sorted(devices)
...
class Nf4MarigoldOffloadAdapter:
    ...
    def _validate_effective_policy(self) -> None:
        ...
        if not vae["execution_hook_devices"] or not all(
            _is_cuda(device) for device in vae["execution_hook_devices"]
        ):
            raise RuntimeError("NF4 Marigold VAE execution hook is missing")
